    ...
    if(!empty($utilreset->email($posttruser))){
        ...
        foreach($usermodal->listaruser($posttruser) as $fbuscaruser){
            if($fbuscaruser['statususer'] == 2){
                echo "<code>Tu cuenta esta suspendida : comunicate a contacto@ibrokergo.com</code>";
            }else if($fbuscaruser['statususer'] == 3){
                echo "<code>Tu cuenta fue eliminada : comunicate a contacto@ibrokergo.com</code>";
                ...
                if($insert == 0){
                    echo "Hubo un error , intentalo más tarde o envia un correo a contacto@ibrokergo.com";
                }else if($insert == 1){
    ...
    if(!empty($utilreset->email($posttruser))){
        ...
        foreach($usermodal->listaruser($posttruser) as $fbuscaruser){
            if($fbuscaruser['statususer'] == 2){
                ...
                $envioemail = $emailreset->emailregister($posttruser,$fbuscaruser['nameandlast'],$posttruser,2,$utilreset->passaleatorio(),$utilreset->fecha());
