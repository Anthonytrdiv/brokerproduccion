...
// Funcion que corta un texto - Según criterio hasta 82 carácteres como máximo
    function cortartexto($textocort){
        ...
        if($totletras > 80){
            $cortadoletter = substr($textocort, 0, 80)." ...";

        }else if($totletras <= 80){
           
            $cortadoletter1=$textocort.$mensajep;
            $cortadoletter = substr($cortadoletter1, 0, 80)." ...";
 ...
 // Herramienta que sirve para contactar whatsapp
    function enviarwhatsapp($nombrescom,$servicio,$detalleserv,$cel){        
        ...
        $bienvenida=",vengo de la pagina IBrokerGo! ";
        $converd = new utilsphp();
        $converd->convertircawhat($bienvenida);
        $data2 = ",necesito preguntarte sobre tu servicio de .:";
        $data3 = ",Detalle .: ";
        echo $link=$flagwhatsapp.$cel."&text=Soy%20".$converd->convertircawhat($nombrescom).$converd->convertircawhat($bienvenida).$converd->convertircawhat($data2).$converd->convertircawhat($servicio).$converd->convertircawhat($data3).$converd->convertircawhat($detalleserv);
